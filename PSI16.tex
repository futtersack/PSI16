...
MP~\cite{Stangeby84, Hutchinson87,Hutchinson88,Matthews89,Hutchinson91,Chung12}
...
being the normalized perpendicular velocity and $\eta_H$, the shear 
viscosity introduced by Hutchinson, the term differencing his model 
to the one of Stangeby. 

The essence of these one-dimentional approximation is to consider the 
...
length of the probe. 
About the momentum source, that Hutchinson resumes 
as~\cite{Hutchinson02}:
\begin{gather}
...
the inviscid case of Stangeby, recovered with by $\alpha=0$, reduces to 
$S_\Gamma=MS_N$, representing the change in momentum of the flow in the 
flux tube due to the entrance of ions with an unperturbed velocity of 
$M_\infty$. 

However, it should be pointed out that in the case of an uncentered probe, 
...
\begin{equation}
N\left(x,z\right)=n\left(z\right)\exp\left(-\frac{x}{\lambda_\text{SOL}}\right)
\qquad\lambda_\text{SOL}=\sqrt{\frac{D_\perp L_\parallel}{c_s}}
\end{equation}
...
\begin{figure}
\begin{minipage}{68mm}
\includegraphics[width=\linewidth]{figures/PartBalance_middle.pdf} \caption{3D view
of $J_\parallel$ from the top, where the probe is located. The white/transparent
color indicates zero current. The bottom plan shows the XY density map in strong
SOL electrostatic turbulence.}
\label{fig:2}
...
HPC resources of Aix-Marseille Université financed by the project Equip@Meso
